...
@pytest.fixture
def mock_mne():
    with patch("vireon.plugins.datasets.fif_reader.mne") as mock_mne_lib:
        mock_raw = MagicMock()
...
def test_fif_reader_with_mne(mock_mne, monkeypatch):
    monkeypatch.setattr("vireon.plugins.datasets.fif_reader.HAS_MNE", True)
...
def test_fif_reader_without_mne(monkeypatch):
    monkeypatch.setattr("vireon.plugins.datasets.fif_reader.HAS_MNE", False)
