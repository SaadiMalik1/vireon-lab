...
from vireon.plugins.ble.true_ble import TrueBLEClient

@pytest.mark.asyncio
async def test_true_ble_connect_success():
    with patch("vireon.plugins.ble.true_ble.BleakScanner.find_device_by_address", new_callable=AsyncMock) as mock_scanner:
        mock_device = MagicMock()
        mock_device.name = "TestDevice"
        mock_scanner.return_value = mock_device

        with patch("vireon.plugins.ble.true_ble.BleakClient", autospec=True) as MockClient:
