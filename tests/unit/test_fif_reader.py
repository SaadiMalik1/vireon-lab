...
@pytest.fixture
def mock_mne():
    with patch("vireon_lab.providers.datasets.fif_reader.mne", create=True) as mock_mne_lib:
        mock_raw = MagicMock()
...
def test_fif_reader_with_mne(mock_mne, monkeypatch):
    monkeypatch.setattr("vireon_lab.providers.datasets.fif_reader.HAS_MNE", True, raising=False)
...
def test_fif_reader_without_mne(monkeypatch):
    monkeypatch.setattr("vireon_lab.providers.datasets.fif_reader.HAS_MNE", False, raising=False)
