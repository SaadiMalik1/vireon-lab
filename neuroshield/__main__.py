...
def sbom(output):
    ...
    from neuroshield.core.sbom import generate_sbom, save_sbom, print_sbom_summary
...
def compliance_report(output):
    ...
    from neuroshield.core.compliance import generate_compliance_report, save_compliance_report, print_compliance_report
...
def audit_spdf():
    ...
    from neuroshield.core.spdf_auditor import SPDFAuditor
...
def monitor():
    ...
    from neuroshield.plugins.reports.vulnerability_monitor import VulnerabilityMonitor
...
@click.option('--protocol', type=click.Choice(['neuroshield', 'brainflow']), default='neuroshield', help='Protocol to fuzz')
...
def fuzz(iterations, secure, verbose, protocol, seed, output):
    ...
    from neuroshield.core.fuzzer import ProtocolFuzzer, BrainFlowFuzzer, print_fuzz_report, save_fuzz_report
...
def stride(output, markdown):
    """Auto-generate a STRIDE threat model for the NeuroShield platform."""
    from neuroshield.core.stride import generate_stride_model, print_stride_summary, save_stride_model, render_stride_markdown
...
def ctf_list():
    ...
    from neuroshield.ctf.engine import ChallengeRunner, print_challenge_list
...
def ctf_start(challenge_id):
    ...
    from neuroshield.ctf.engine import ChallengeRunner, print_challenge_result
...
def simulate_poisoning(dataset, target_class, ratio, mode):
    ...
    import numpy as np
    from neuroshield.attacks.poisoning import LabelFlippingPoisoner, CleanLabelBackdoorPoisoner
