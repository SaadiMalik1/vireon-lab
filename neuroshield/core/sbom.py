"""
NeuroShield SBOM Generator — CycloneDX 1.5 Software Bill of Materials.
...
def generate_sbom(project_root: str, output_format: str = "json") -> Dict[str, Any]:
    """
    Generate a CycloneDX 1.5 SBOM for the NeuroShield project.
    ...
    project_name = project.get("name", "neuroshield")
    ...
    for group_name, group_deps in project.get("optional-dependencies", {}).items():
        for dep in group_deps:
            # Skip self-references like "neuroshield[...]"
    ...
    for dep_str in all_deps:
        ...
        component = {
            ...
            "scope": "required",
            "properties": [
                {"name": "neuroshield:ecosystem", "value": "python"},
                {"name": "neuroshield:version_spec", "value": parsed["version_spec"]}
            ]
        }
    ...
    for pkg in cargo_packages:
        ...
        component = {
            ...
            "scope": "required",
            "properties": [
                {"name": "neuroshield:ecosystem", "value": "rust"},
            ]
        }
    ...
    for req in build_requires:
        ...
        if parsed["name"] not in seen_names:
            ...
            components.append({
                ...
                "properties": [
                    {"name": "neuroshield:ecosystem", "value": "python"},
                    {"name": "neuroshield:role", "value": "build-system"}
                ]
            })
    ...
    bom = {
        ...
        "metadata": {
            ...
            "tools": {
                "components": [
                    {
                        "type": "application",
                        "name": "neuroshield-sbom-generator",
                        "version": project_version,
                        "description": "NeuroShield built-in SBOM generator for FDA 524B compliance"
                    }
                ]
            },
            "component": {
                ...
                "properties": [
                    {"name": "neuroshield:python_requires", "value": project.get("requires-python", ">=3.10")},
                    {"name": "neuroshield:license", "value": "MIT"},
                    {"name": "neuroshield:fda_device_class", "value": "cyber_device_524b"}
                ]
            },
            "lifecycles": [
                {"phase": "build"},
        ...
        "properties": [
            {"name": "neuroshield:sbom_purpose", "value": "FDA Section 524B premarket submission"},
            {"name": "neuroshield:component_count", "value": str(len(components))},
            {"name": "neuroshield:ecosystems", "value": "python,rust"}
        ]
    }
...
def print_sbom_summary(bom: Dict[str, Any]) -> None:
    ...
    print("=" * 60)
    print(" NeuroShield SBOM Summary (CycloneDX 1.5)")
    print("=" * 60)
