...
| **Denial of Service** | Denying or degrading service to users. | Battery drain attack (Peukert's Law). | Firmware Fallback Mode, IPS rate limiting. |
| **Elevation of Privilege** | Gaining capabilities without proper authorization. | Exploiting debugger interface to read/write memory. | Memory isolation (Bootloader vs App regions). |
